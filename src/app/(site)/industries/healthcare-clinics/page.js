...
const SLUG = 'healthcare-clinics-industry'

const OPTS = {
    hub: { label: 'Industries', href: '/industries' },
    relatedLabel: 'healthcare',
    breadcrumb: [
        { name: 'Home', href: '/' },
        { name: 'Industries', href: '/industries' },
    ],
    utm: 'healthcareclinics',
    signature: true,
}
...
export async function generateMetadata() {
    ...
    if (!svc) return {}
    const url = `${SITE}/industries/healthcare-clinics/`
...
export default async function HealthcareClinicsIndustryPage() {
    const svc = await getContentItem('deepPages', SLUG)
    if (!svc) notFound()
